...
try {
  ...
  assert.deepEqual(levelFor(0), { tier: "iron", division: 1, name: "Iron 1", xp: 0, floor: 0, next: 10, nextName: "Iron 2" });
  assert.equal(levelFor(9).name, "Iron 1");
  assert.equal(levelFor(10).name, "Iron 2");
  assert.equal(levelFor(50).name, "Bronze 1");
  assert.equal(levelFor(1_499).name, "Silver 2");
  assert.equal(levelFor(599_999).name, "Diamond 2");
  assert.deepEqual(levelFor(600_000), { tier: "diamond", division: 3, name: "Diamond 3", xp: 600_000, floor: 600_000, next: 1_000_000, nextName: "Bouncer" });
  assert.equal(levelFor(999_999).name, "Diamond 3", "Just short of 100,000 SOL is still Diamond");
  assert.deepEqual(levelFor(1_000_000), { tier: "bouncer", division: null, name: "Bouncer", xp: 1_000_000, floor: 1_000_000, next: null, nextName: null });
  assert.equal(experienceFromWagered(100_000 * SOL), 1_000_000, "Bouncer is 100,000 SOL wagered");
  assert.equal(levelFor(-5).name, "Iron 1", "Bad input stays at the bottom");
  assert.equal(experienceFromWagered(1 * SOL), 10, "1 XP per 0.1 SOL");
  assert.equal(experienceFromWagered(0.15 * SOL), 1, "Partial points round down on the total");
  ...
  // 1 + 0.5 (a tie still counts) + 0.25 + 0.25 (deleted tournament) = 2 SOL = 20 XP.
  const liv = await playerLevel(LIV);
  assert.equal(liv.xp, 20, "Settled matches, ties and closed tournaments count; the rest does not");
  assert.equal(liv.name, "Iron 2");
  ...
  assert.equal((await playerLevel(LIV)).name, "Bronze 2", "10 SOL wagered is 100 XP");

  const profile = await publicProfile("Liv", MAX);
  assert.equal(profile.level.xp, 100, "Profiles carry the rank");
  const leaders = await matches.leaderboard(null, "devnet");
  const livRow = leaders.find((l) => l.name === "Liv");
  assert.equal(livRow.level.name, "Bronze 2", "Leaderboard rows carry the rank");
  assert.ok(!("wagered" in livRow) && !JSON.stringify(leaders).includes("private"), "No raw totals or player IDs in the leaderboard");
  assert.equal((await matches.leaderboard(null, "gems")).find((l) => l.name === "Liv").level.xp, 100, "Gem leaderboard shows the same SOL rank");

  console.log("PASS: ranks (ladder and thresholds, 1 XP per 0.1 SOL, Bouncer at 100,000 SOL, settled matches and ties, closed and deleted tournaments minus refunds, no XP for unsettled or cancelled matches, upcoming tournaments, tips, withdrawals or gems, profile and leaderboard ranks without IDs).");
} finally {
  close();
}
