...
- Fee-free devnet SOL tips between funded Ricochet balances, with recipient/amount review, atomic debit and credit, duplicate-request protection, and sent/received wallet history. Tips do not submit a separate on-chain transfer or enter match PNL.
...
Gem matches have no house fee: winners receive the entire two-entry pot, and an unjoined cancellation refunds the full entry. Devnet SOL winners receive 1.76 times their stake; the house receives 12% of both entries. Ties refund both entries fully. One forfeit loses to a completed opponent; if both forfeit, scores decide. An unjoined devnet cancellation refunds the entry minus the 12% fee. Previously settled results retain their recorded payouts and fees. Open matches do not expire and reserved entries are not withdrawable.
...
Apply migration `0004_public_player_ids` before deploying public profiles and tips. It backfills random public IDs and assigns one to every new player; existing balances and match history are preserved. `tests/profiles-tips.test.mjs` verifies all-time PNL, historical results, profile privacy, tip idempotency, concurrent spending, failed-credit rollback, wallet history and devnet-only operation.
