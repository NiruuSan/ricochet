...
export function launchStatus(settings: PaymentSettings) {
  return {
...
export function parseSol(value: unknown): number {
  ...
  const amount = BigInt(whole) * BigInt(1000000000) + BigInt(fraction.padEnd(9, "0"));
...
export function validateOperationId(value: unknown): string {
  if (typeof value !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) {
    throw new PaymentError("A valid operation ID is required.");
  }
