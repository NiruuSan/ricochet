...
export function safePaymentError(error:unknown):string {
 ...
 if(error instanceof Error&&/SQLITE|UNIQUE|constraint|cash balance insufficient/i.test(error.message))return 'A payment is already pending or your balance changed. Refresh your wallet before retrying.';
